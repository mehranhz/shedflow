  ...
  private mapException(exception: unknown): {
    ...
    if (exception instanceof EntityNotFoundError) {
      return {
        ...
        details: { entity: exception.entityName },
      };
    }

    if (exception instanceof UniqueConstraintError) {
      const slugTaken = exception.fields.some((field) =>
        field.toLowerCase().includes('slug'),
      );
      const emailTaken = exception.fields.includes('email');
      return {
        status: HttpStatus.CONFLICT,
        code: slugTaken ? 'SLUG_TAKEN' : emailTaken ? 'EMAIL_TAKEN' : 'CONFLICT',
        message: exception.message,
    ...
    if (exception instanceof ConflictException) {
      const message = this.httpMessage(exception, exception.message);
      const payload = this.httpPayload(exception);
      if (payload.code === 'IDEMPOTENCY_MISMATCH') {
        return {
          status: HttpStatus.CONFLICT,
      ...
      if (payload.code === 'SLUG_TAKEN' || /slug/i.test(message)) {
        return {
          status: HttpStatus.CONFLICT,
          code: 'SLUG_TAKEN',
          message,
      ...
      const emailTaken = /email/i.test(message);
      return {
        status: HttpStatus.CONFLICT,
        code: emailTaken ? 'EMAIL_TAKEN' : 'CONFLICT',
        message,
        details: this.detailsFromPayload(payload),
      };
    }
...
export function buildErrorEnvelope(
  ...
  return new AllExceptionsFilter().describe(exception, requestId);
}
