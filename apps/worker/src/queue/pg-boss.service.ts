...
import { eventQueueName, OUTBOX_RELAY_QUEUE, PGBOSS_SCHEMA } from '../outbox/outbox.constants';
...
export class PgBossService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    await boss.start();
    await boss.createQueue(OUTBOX_RELAY_QUEUE, { policy: 'singleton' });
  ...
  async enqueueDomainEvent(job: DomainEventJob): Promise<void> {
    ...
    await this.client().send(queue, job, { singletonKey: job.eventId });
  }
